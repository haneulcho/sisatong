<?php
	...
	class sisatonglist extends WidgetHandler {
		...
		function proc($args) {
			...
			if(!$args->module_srls){
				...
				$output = executeQueryArray('widgets.sisatonglist.getMids', $obj);
				if($output->data) {
			...
			$output = executeQueryArray('widgets.sisatonglist.getCategories',$obj);
			...
			while(true) {
				...
				$obj->page = $pagecount;
				$output = executeQueryArray('widgets.sisatonglist.getNewestDocuments', $obj);
				// 더이상 결과가 없으면 break
			...
			if($getcount > 0) {
				...
				for($i=0,$c=count($document_srls);$i<$c;$i++) {
					...
					$content_item = new sisatonglistItem( $args->module_srls_info[$module_srl]->browser_title );
					...
					$sisatonglist_items[] = $content_item;
				}

				$sisatonglist_items[0]->setFirstThumbnailIdx($first_thumbnail_idx);
			}
			...
			$widget_info->sisatonglist_items = $sisatonglist_items;
	...
	class sisatonglistItem extends Object {
		...
		function sisatonglistItem($browser_title=''){
