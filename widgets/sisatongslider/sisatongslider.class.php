<?php
	/**
	 * @class sisatongslider
	 * @author 이한결 (mrrays@naver.com)
	 * @brief 시사통 슬라이더를 출력하는 위젯
	 **/

	class sisatongslider extends WidgetHandler {
		...
		function proc($args) {
			...
			if(!$args->module_srls){
				...
				$output = executeQueryArray('widgets.sisatongslider.getMids', $obj);
				if($output->data) {
			...
			$output = executeQueryArray('widgets.sisatongslider.getCategories',$obj);
			...
			while(true) {
				...
				$obj->page = $pagecount;
				$output = executeQueryArray('widgets.sisatongslider.getNewestDocuments', $obj);
				// 더이상 결과가 없으면 break
			...
			if($getcount > 0) {
				...
				for($i=0,$c=count($document_srls);$i<$c;$i++) {
					...
					$content_item = new sisatongsliderItem( $args->module_srls_info[$module_srl]->browser_title );
					...
					$sisatongslider_items[] = $content_item;
				}

				$sisatongslider_items[0]->setFirstThumbnailIdx($first_thumbnail_idx);
			}
			...
			$widget_info->sisatongslider_items = $sisatongslider_items;
	...
	class sisatongsliderItem extends Object {
		...
		function sisatongsliderItem($browser_title=''){
