<?php
	/**
	 * @class sisatongsliderM
	 * @author 이한결 (mrrays@naver.com)
	 * @brief 시사통 슬라이더를 출력하는 위젯
	 **/

	class sisatongsliderM extends WidgetHandler {
		...
		function proc($args) {
			...
			if(!$args->module_srls){
				...
				$output = executeQueryArray('widgets.sisatongsliderM.getMids', $obj);
				if($output->data) {
			...
			$output = executeQueryArray('widgets.sisatongsliderM.getCategories',$obj);
			...
			while(true) {
				...
				$obj->page = $pagecount;
				$output = executeQueryArray('widgets.sisatongsliderM.getNewestDocuments', $obj);
				// 더이상 결과가 없으면 break
			...
			if($getcount > 0) {
				...
				for($i=0,$c=count($document_srls);$i<$c;$i++) {
					...
					$content_item = new sisatongsliderMItem( $args->module_srls_info[$module_srl]->browser_title );
					...
					$sisatongsliderM_items[] = $content_item;
				}

				$sisatongsliderM_items[0]->setFirstThumbnailIdx($first_thumbnail_idx);
			}
			...
			$widget_info->sisatongsliderM_items = $sisatongsliderM_items;
	...
	class sisatongsliderMItem extends Object {
		...
		function sisatongsliderMItem($browser_title=''){
