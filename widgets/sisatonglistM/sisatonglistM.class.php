<?php
	...
	class sisatonglistM extends WidgetHandler {
		...
		function proc($args) {
			...
			// 썸네일 가로 크기
			if(!$args->thumbnail_width) $args->thumbnail_width = 89;

			// 썸네일 세로 크기
			if(!$args->thumbnail_height) $args->thumbnail_height = 63;
			...
			if(!$args->module_srls){
				...
				$output = executeQueryArray('widgets.sisatonglistM.getMids', $obj);
				if($output->data) {
			...
			$output = executeQueryArray('widgets.sisatonglistM.getCategories',$obj);
			...
			while(true) {
				...
				$obj->page = $pagecount;
				$output = executeQueryArray('widgets.sisatonglistM.getNewestDocuments', $obj);
				// 더이상 결과가 없으면 break
			...
			if($getcount > 0) {
				...
				for($i=0,$c=count($document_srls);$i<$c;$i++) {
					...
					$content_item = new sisatonglistMItem( $args->module_srls_info[$module_srl]->browser_title );
					...
					$sisatonglistM_items[] = $content_item;
				}

				$sisatonglistM_items[0]->setFirstThumbnailIdx($first_thumbnail_idx);
			}
			...
			$widget_info->sisatonglistM_items = $sisatonglistM_items;
	...
	class sisatonglistMItem extends Object {
		...
		function sisatonglistMItem($browser_title=''){
